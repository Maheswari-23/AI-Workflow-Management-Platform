...
export default function ToolsMainPanel({selectedTool,onSave}){
  ...
  useEffect(()=>{if(selectedTool)setFormData({name:selectedTool.name||'',type:selectedTool.type||'api',description:selectedTool.description||'',endpoint:selectedTool.endpoint||''});},[selectedTool]);
  const handleChange=(e)=>setFormData(p=>({...p,[e.target.name]:e.target.value}));
  const handleSave=async(e)=>{e.preventDefault();setIsSaving(true);await new Promise(r=>setTimeout(r,500));onSave({...selectedTool,...formData});setIsSaving(false);};
